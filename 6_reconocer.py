...
def reconocer_rostros():
    ...
    subcarpetas_entrenamiento = [nombre for nombre in os.listdir(ruta_entrenamiento) if os.path.isdir(os.path.join(ruta_entrenamiento, nombre))]
    nombres = {int(id): nombre for id, nombre in enumerate(subcarpetas_entrenamiento)}
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            # Verificar si el ID está en el diccionario de nombres
            if conf < 80 and id_ in nombres:
                nombre = nombres[id_]
            else:
                nombre = "Desconocido"
